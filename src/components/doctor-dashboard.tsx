...
export function DoctorDashboard() {
  ...
  return <div className="doctor-shell">
    ...
    <main className="main-shell">
      ...
      {demoStep > 0 && <div className="demo-guide"><div className="demo-progress">{demoSteps.map((_, index) => <span key={index} className={index < demoStep ? "done" : ""} />)}</div><div><span>{demoSteps[demoStep - 1].kicker}</span><strong>{demoSteps[demoStep - 1].title}</strong><p>{demoSteps[demoStep - 1].copy}</p></div><button className="button button-light" onClick={advanceDemo}>{demoStep === demoSteps.length ? "Finish tour" : "Next wow factor"}<ArrowRight size={16} /></button><button className="icon-button" onClick={() => setDemoStep(0)} aria-label="Close demo guide"><X size={17} /></button></div>}
...
function Overview({ scenario, repaired, scanning, showExplanation, onScan, onExplain, onOpen }: { scenario: Scenario; repaired: boolean; scanning: boolean; showExplanation: boolean; onScan: () => void; onExplain: () => void; onOpen: (view: View) => void }) {
  ...
  return <>
    ...
    <div className="overview-lower">
      ...
      <Panel className="demo-card"><span className="mini-label">JUDGE MODE</span><h2>One failure.<br />Four lenses.</h2><p>Use the synchronized diagnostic loop for a crisp three-minute story.</p><ol><li><span>1</span>Score the surface</li><li><span>2</span>Trace the failure</li><li><span>3</span>Repair and retest</li><li><span>4</span>Compare the twin</li></ol><button className="button button-light" onClick={() => onOpen("xray")}>Begin with X-Ray <ArrowRight size={15} /></button></Panel>
    </div>
  </>;
}
